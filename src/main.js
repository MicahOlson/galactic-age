...
$(document).ready(() => {
  ...
  $("#seeAllLink").click(() => {
    ...
    for (const planet in user.galacticAges) {
      $("#statsTable").append(`<tr id="appendedTableRows"><td>${planet}</td><td>${Math.round(user.galacticAges[planet])}</td><td>${Math.round(user.galacticLifeExpectancies[planet])}</td><td>${Math.round(user.galacticLifeExpectancies[planet] - user.galacticAges[planet])}</td></tr>`);
    }
  });
});